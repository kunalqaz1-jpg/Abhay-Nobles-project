  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setTimeout(() => {
      if (username === "admin" && password === "1234") {
        ...
        setError("Invalid credentials. Demo: admin / 1234");
        setLoading(false);
      }
    }, 600);
  }
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .erp-login-wrap::before{content:'';position:fixed;inset:-50%;background:radial-gradient(ellipse at 70% 30%,rgba(201,168,76,0.12) 0%,transparent 50%),radial-gradient(ellipse at 30% 80%,rgba(30,48,96,0.5) 0%,transparent 45%);animation:bgDrift 18s ease-in-out infinite alternate;pointer-events:none;}
      ...
      <div className="erp-login-wrap">
        ...
        <div className="erp-card">
          <div className="erp-brand">
            ...
            <span>Admin / Principal Portal</span>
          </div>
          {error && <p className="erp-error">{error}</p>}
          <form onSubmit={handleSubmit} noValidate>
            <div className="erp-group">
              <label htmlFor="al-user">Username</label>
              <input id="al-user" type="text" placeholder="e.g. admin" value={username} onChange={(e) => setUsername(e.target.value)} required />
            </div>
            <div className="erp-group">
              <label htmlFor="al-pw">Password</label>
              <input id="al-pw" type="password" placeholder="Enter password" value={password} onChange={(e) => setPassword(e.target.value)} required />
            </div>
            <button type="submit" className="erp-btn" disabled={loading}>{loading ? "Signing in…" : "Login"}</button>
          </form>
          <p className="erp-hint">Demo: <strong>admin</strong> / <strong>1234</strong></p>
